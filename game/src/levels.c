...
const LevelInfo levels[MAX_LEVELS] = {
    ...
    },
    {
        ...
        .bg = {
            ...
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY},
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY},
            {UNFIL, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL}
        }
    },
    {
        .moves = 10,
        ...
        .objScore = 40,
        .bg = {
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL},
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL},
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL},
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY},
            {EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL, EMPTY, EMPTY, EMPTY, EMPTY},
            {EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL, EMPTY, EMPTY, EMPTY, EMPTY},
            {EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY},
            {UNFIL, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL},
            {UNFIL, UNFIL, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL},
            {UNFIL, UNFIL, UNFIL, EMPTY, EMPTY, EMPTY, EMPTY, UNFIL, UNFIL, UNFIL}
        }
    }
};
...
/**
 * Comprueba si se han cumplido los objetivos del nivel actual.
 */
bool checkWinCondition(LevelInfo* level) {
    switch(level->objective) {
        case OBJ_DESTROY_CORPT:
            return (level->objCorptRemaining == 0);
        case OBJ_DESTROY_ITEM:
            return (level->objItemRemaining == 0);
        case OBJ_GET_SCORE:
            return (level->score >= level->objScore);
        default:
            return false;
    }
}
...
void resetLevel(LevelInfo* level) {
    // 1. Limpiar contadores básicos
    level->score = 0;
    level->moves = 0;
    
    // 2. Limpiar objetivos y sus contadores específicos
    level->objective = NUM_OBJECTIVES; // Valor seguro por defecto
    level->objCorptRemaining = 0;
    level->objItemRemaining = 0;
    level->objItemType = ITEM_EMPTY;
    level->objScore = 0;

    // 3. Limpiar tableros (rellenar con vacío/seguro)
    for (int i = 0; i < MAX_Y_SPRITES; i++) {
        for (int j = 0; j < MAX_X_SPRITES; j++) {
            level->bg[i][j] = UNFIL;      // Fondo a 'Sin rellenar' por seguridad
            level->fg[i][j] = ITEM_EMPTY; // Frente a 'Vacío'
        }
    }
}
